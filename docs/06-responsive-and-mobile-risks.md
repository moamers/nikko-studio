...
|---|---|---|---|---|
| 1 | [R8](#r8--service-card-divider-points-the-wrong-way-): service card Column C's divider is no longer an absolutely-positioned `border-left` fired unconditionally. It is a `border-block-start` (top rule, static flow) by default, switching to the original `border-inline-start` (absolutely-positioned, inset 18px top/bottom) only inside `@container nk-service-card (min-width: 680px)`. 680px is not arbitrary: `grid-template-columns: repeat(auto-fit, minmax(min(100%,340px),1fr))` collapses from two tracks to one at exactly `2 × 340px` of available width, so the query threshold reproduces the grid's own arithmetic. | R8 — the source's absolute divider survives unchanged into the single-column stacked layout as a stray vertical dashed line down the left edge of a full-width block. | `Services.astro` (`#services`), all widths — the container query is scoped to each card, not a global breakpoint | Engineering (per this task's explicit instruction) |
| 2 | [R4](#r4--hero-h1-is-crushed-to-218-px-), option (a): below 900px the hero drops the `calc(100% + 56px - min(38vw,430px))` reservation on the ticket bar, H1 and body row entirely, and the collage leaves the absolutely-positioned overlap to become a normal-flow, full-width figure after the CTA row instead. At 900px+ the source's overlap and reservation formula are unchanged. | R4 — the reservation resolves to ~218px at 390px width, crushing the 130-character headline to ~7 characters/line. | `Hero.astro`, `< 900px` | Engineering (per this task's explicit instruction) |
| 3 | [R7](#r7--ticket-bar-wraps-into-a-ragged-shadow-): below 480px the accent-filled "Creative design studio" ticket cell is given `flex: 1 1 100%`, forcing it onto its own full-width row under "London" / "UAE" — a deliberate two-row block rather than the ragged wrap `flex-wrap` produces on its own. | R7 — three cells need ~363px in a 310px row; an undirected wrap leaves the hard `8px 8px 0` offset shadow tracing an uneven silhouette. | `Hero.astro`, `< 480px` | Engineering (per this task's explicit instruction) |
| 4 | ~~Hero collage: the source's hover-settle micro-interaction (`rotate(2.6deg)` → `rotate(1.2deg) translateX(-34px)` on hover) is dropped.~~ **REVERSED 2026-08-21 — the interaction is restored as specified**, `420ms cubic-bezier(0.3,1,0.4,1)`, inside `@media (min-width: 900px) and (hover: hover) and (prefers-reduced-motion: no-preference)`. `pointer-events: auto` is returned to the collage in that branch only; `aria-hidden="true"` is unchanged, since `pointer-events` is not an accessibility-tree property and the decorative-layer rule is about what assistive tech is told, not about what a mouse can touch. Below 900px the collage is a full-width figure in normal flow with no rotation to settle from, and keeps `pointer-events: none`. | The original reading treated `pointer-events: none` as inseparable from `aria-hidden`. It is not — the handoff gives this element both, deliberately. The founder identified the missing slide directly ("that picture was supposed to have a slide effect when you hover on it"), which is P0 rank 1 and outranks the earlier engineering call. | `Hero.astro`, `≥ 900px`, fine pointer, motion allowed | Founder, 2026-08-21 |
| 5 | [R3](#r3--language-tiles-text-does-not-fit-), recommendation 1: below 768px the three language tiles stop being flex siblings at `3/4` and stack vertically at full width, `aspect-ratio: 16/10`. The independent parallax speeds (`nk-para`/`nk-para-fast`/`nk-para-rev`) stay wired to the same three tiles; the visual variance is simply less noticeable full-width. | R3 — three `3/4` tiles at 390px compute to ~99px wide with ~71px of text width for a 56-character headline (~7.6 chars/line, ~148px of needed content in a 132px tile). | `Language.astro`, `< 768px` | Engineering (per this task's explicit instruction) |
| 6 | [R5](#r5--hover-scraps-have-no-touch-equivalent-): the Attention heading's hover-scrap fan is `display:none` outside `@media (hover:hover) and (min-width:1024px)`, exactly as recommended, and the four trigger words are additionally made focusable (`tabindex="0"`) with the reveal firing on `:focus-within` as well as `:hover`, since the source's `onMouseEnter`-only spans have no keyboard path at all. | R5 — no hover on touch hides the interaction entirely, and the lane's `min(52vw,360px)` geometry at `left:26%` etc. overflows a phone viewport if it stays laid out. | `Attention.astro`, `< 1024px` or no `hover:hover` | Engineering (per this task's explicit instruction) |
| 7 | Hero analogue signal layers rest at their first keyframe rather than at full strength: `.nk-hero__scan-lines` at `opacity: 0.46` (`nk-flick` 0%), `.nk-hero__scan-sweep` and `.nk-hero__tear` at `opacity: 0`, with the sweep returned to 1 only where the animation runs. | The source declares no resting opacity because in the prototype the animation always runs. Under `prefers-reduced-motion` that left `nk-tear` — a flash that occupies 8% of a 9s cycle — parked across the collage as a permanent 5px coral bar, and the scan sweep parked as a static bright band. Reduced motion is supposed to remove the movement, not leave one frame of it nailed up. | `Hero.astro`, `prefers-reduced-motion: reduce` | Engineering (P1, docs/05 § Reduced motion) |
| 8 | The full-page grain overlay is absent below 768px and on coarse pointers, per [R6](#r6--full-page-grain-overlay-is-expensive-)'s recommendation, and behind `@supports (mix-blend-mode: multiply)` per [P3](./02-engineering-principles.md#p3--progressive-enhancement-in-layers-in-that-order). Analogue's other tells — image grade, hero signal, H1 misregistration — are unconditional, so the mode still reads on a phone. | R6: a viewport-sized `multiply` layer forces a full-screen composite every frame, and a 1px/3px gradient at 0.13 opacity is sub-pixel on a 3x display. We would be paying the most expensive effect in the design for one nobody can see. | `grain.css`, `< 768px` or `(pointer: coarse)` or no blend-mode support | Engineering (per R6's own recommendation) |

| 9 | The analogue/digital scroll-drift is **inverted**, and the resting state with it. The handoff opens analogue and drifts to digital past `--nk-mode-flip` (52% depth); the site now opens **digital** and drifts to **analogue** past the same threshold. The mechanism, the threshold and the pinning behaviour are unchanged — only the ends are swapped. `:root` therefore carries the digital values and `:root[data-mode='analogue']` the analogue ones (`tokens.css`, `cursor.css`, `Hero.astro`), the reverse of the original inversion. | The founder read the original direction as a fault: a first-time visitor met the grain immediately and then watched it disappear mid-scroll, which reads as a rendering failure rather than as atmosphere. Verified first that it was not a bug — the dial label, `data-mode` and the grain all changed together, correctly, at exactly 52%. Presented as a design choice and the founder chose to open on the clean state. Flipping the resting state is not optional once the default flips: measured on a 6× throttled CPU, leaving `:root` analogue painted grain for ~140ms before the script removed it — a flash of an effect that is not meant to be there yet. It also means a no-JS visitor gets the clean, cheaper render. | Site-wide, unpinned (`auto`) mode only | Founder, 2026-08-21 |
