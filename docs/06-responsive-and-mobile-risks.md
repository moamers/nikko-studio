...
| 4 | Hero collage: the source's hover-settle micro-interaction (`rotate(2.6deg)` → `rotate(1.2deg) translateX(-34px)` on hover) is dropped. The collage is `aria-hidden="true"` and, per this task's decorative-layer rule, `pointer-events: none`, which makes it unable to receive a hover event at all. | Direct conflict between the source's hover affordance on a decorative element and the `aria-hidden` + `pointer-events: none` rule for decorative layers; the latter took precedence. | `Hero.astro`, all widths | Engineering (per this task's explicit instruction) |
